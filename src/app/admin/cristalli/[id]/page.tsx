...
export default function AdminCristallo() {
  const router = useRouter();
  const params = useParams();
  const isNuovo = params.id === "nuovo";

  const [form, setForm] = useState<CristalloForm>(EMPTY);
  const [original, setOriginal] = useState<CristalloForm>(EMPTY);
  ...
  useEffect(() => {
    ...
    async function carica() {
      const snap = await getDoc(doc(db, "cristalli", params.id as string));
      if (!snap.exists()) { router.push("/admin/cristalli"); return; }
      const d = snap.data();
      const f: CristalloForm = {
        id: snap.id,
        nome: d.nome || "",
        codice: d.codice || "",
  ...
  function update<K extends keyof CristalloForm>(key: K, value: CristalloForm[K]) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await setDoc(doc(db, "cristalli", form.id), data, { merge: !isNuovo });
      setOriginal(form);
      if (isNuovo) router.push(`/admin/cristalli/${form.id}`);
    } catch (e) {
  ...
  async function handleUpload(file: File) {
    setUploading(true);
    try {
      const fileRef = ref(storage, `cristalli/${form.id || "tmp"}_${Date.now()}_${file.name}`);
  ...
  async function handleDelete() {
    try {
      ...
      await deleteDoc(doc(db, "cristalli", form.id));
      router.push("/admin/cristalli");
    } catch (e) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <button className={styles.backBtn} onClick={() => router.push("/admin/cristalli")}>← Cristalli</button>
        <h1 className={styles.title}>{isNuovo ? "Nuovo cristallo" : form.nome || "Cristallo"}</h1>
        <div className={styles.headerActions}>
          {!isNuovo && (
            <button className={styles.deleteBtn} onClick={() => setShowDeleteDialog(true)}>Elimina</button>
          )}
          <button className={styles.cancelBtn} onClick={() => router.push("/admin/cristalli")}>Annulla</button>
      ...
      <div className={styles.body}>
        <div className={styles.col}>
          <div className={styles.section}>
            ...
            <div className={styles.field}>
              <label className={styles.label}>Codice Swarovski</label>
              <input className={styles.input} value={form.codice} onChange={(e) => update("codice", e.target.value)} placeholder="es. 01SWST-204" />
            </div>
            <div className={styles.field}>
      ...
      {showDeleteDialog && (
        <div className={styles.dialogOverlay}>
          <div className={styles.dialog}>
            <h2 className={styles.dialogTitle}>Elimina cristallo</h2>
