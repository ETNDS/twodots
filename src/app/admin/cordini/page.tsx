...
import { getCordini, deleteCordino, ItemColore } from "@/lib/configuratore";
...
export default function Page() {
  ...
  useEffect(() => { getCordini().then((data) => { setItems(data); setLoading(false); }); }, []);

  async function handleDelete(item: ItemColore) {
    await deleteCordino(item.id);
  ...
  return (
    <div>
      <div className={styles.header}><h1 className={styles.title}>Cordini</h1></div>
      <div className={styles.toolbar}>
        ...
        <button className={styles.addBtn} onClick={() => router.push("/admin/cordini/nuovo")}>+ Nuovo cordino</button>
      </div>
      <AdminTable columns={COLUMNS} data={filtrati} loading={loading} onEdit={(i) => router.push(`/admin/cordini/${i.id}`)} onDelete={handleDelete} deleteLabel="cordino" />
    </div>
  );
}
