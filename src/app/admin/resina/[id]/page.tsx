...
export default function AdminResina() {
  const router = useRouter();
  const params = useParams();
  const isNuovo = params.id === "nuovo";

  const [form, setForm] = useState<ResinaForm>(EMPTY);
  const [original, setOriginal] = useState<ResinaForm>(EMPTY);
  ...
  useEffect(() => {
    ...
    async function carica() {
      const snap = await getDoc(doc(db, "coloriResina", params.id as string));
      if (!snap.exists()) { router.push("/admin/resina"); return; }
      const d = snap.data();
      const f: ResinaForm = {
        id: snap.id,
        nome: d.nome || "",
  ...
  function update<K extends keyof ResinaForm>(key: K, value: ResinaForm[K]) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await setDoc(doc(db, "coloriResina", form.id), data, { merge: !isNuovo });
      setOriginal(form);
      if (isNuovo) router.push(`/admin/resina/${form.id}`);
    } catch (e) {
  ...
  async function handleUpload(file: File) {
    setUploading(true);
    try {
      const fileRef = ref(storage, `resina/${form.id || "tmp"}_${Date.now()}_${file.name}`);
  ...
  async function handleDelete() {
    try {
      ...
      await deleteDoc(doc(db, "coloriResina", form.id));
      router.push("/admin/resina");
    } catch (e) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <button className={styles.backBtn} onClick={() => router.push("/admin/resina")}>← Colori resina</button>
        <h1 className={styles.title}>{isNuovo ? "Nuovo colore resina" : form.nome || "Colore resina"}</h1>
        <div className={styles.headerActions}>
          {!isNuovo && (
            <button className={styles.deleteBtn} onClick={() => setShowDeleteDialog(true)}>Elimina</button>
          )}
          <button className={styles.cancelBtn} onClick={() => router.push("/admin/resina")}>Annulla</button>
